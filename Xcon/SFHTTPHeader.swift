...
//            }
    ...
    deinit {
        Xcon.log("HTTPHeader dealloc",level: .Debug)
    }
}
...
open  class  SFHTTPResponseHeader :SFHTTPHeader{
    ...
    open func shouldClose() ->Bool{
        if mode == .ContentLength {
            ...
            Xcon.log("Transfer-Encoding parser will support future",level: .Notify)
            return false
        }
    }
    ...
    deinit {
        Xcon.log("[HTTPRespHeader] deinit",level: .Debug)
    }
}
...
open class  SFHTTPRequestHeader :SFHTTPHeader{
    ...
    deinit {
        Xcon.log("HTTPRequestHeader dealloc",level: .Debug)
    }
    ...
    public override init? (data:Data){
        ...
        if c.count == 3 {
            ...
            //NSLog("url %@",self.Url)
            if self.Url.isEmpty {
                Xcon.log("HTTPRequest \(row) not url  \(c) ",level: .Trace)
            ...
            if self.method == .CONNECT {
                ...
                if let u = NSURL.init(string: self.Url) {
                    if self.method == .POST {
                        Xcon.log("request \(Url)",level: .Trace)
                    }
                    if  let port = u.port {
                    ...
                    if self.method == .POST {
                        Xcon.log("new request \(Url)",level:.Trace)
                    }
                }else {
                    //tcp raw 80
                    self.Url = "http://" + self.Host + self.Url
